...
```js
fetch("https://api.github.com/search/repositories?q=typescript") // fetch the /repositories resource with a "q" parameter
  ...
  .then(
    json =>
      Array.isArray(json.items) && json.items.length > 0 // This API returns an object with a "items" field as an array of objects
        ? Promise.resolve(json.items[0]) // Resolve with the first object if present, an object with a url, name, and id
        : Promise.reject(new Error("No results found.")) // Reject if nothing is present
  )
  .then(data => fetch(data.url + "/pulls?state=open")) // Fetch the associated /pulls resource for a Github repository page, retrieving just those that are open
  ...
  .then(json => console.log(`Found ${json.length} open pull requests in the Typescript Github repository!`)) // Do something with the final result
  .catch(err => console.log("Unable to get Github commit information: " + err));
```
...
The lecture slides [Testing Asynchronous Code in Week 11](https://umamherst.instructure.com/courses/35770/files/17091575?module_item_id=3188915) give a step by step description of how to properly mock `fetch`.
...
Use the following API, <https://cs220.cs.umass.edu/geoCoord/search?q=query+goes+here>, to retrieve your results. The base URL should be `"https://cs220.cs.umass.edu"` and there should be one URL search parameter `q` for the query. This API returns an array of objects containing a `lon`, `lat`, and `importances` field.
...
Use the <https://cs220.cs.umass.edu/currentTemperature/forecast?latitude=40&longitude=40&hourly=temperature_2m&temperature_unit=fahrenheit> API to retrieve your result. It has `latitude` and `longitude` parameters. You should set the `hourly` parameter to `temperature_2m`, and the `temperature_unit` parameter to `fahrenheit`.
...
Use the following API, <https://cs220.cs.umass.edu/universities/search?name=name+query+goes+here>, retrieve your result. It has a `name` parameter to search for universities with a similar name. **If there are no results (the returned JSON is an empty array), resolve to an empty array.**
...
If you would like to cache the results of an API call to an API of your choice, you can use the 220 caching server that we described in the [Rate Limiting Section](#rate-limiting). Here is an example of the URL for caching the [https://bored-api.appbrewery.com](https://bored-api.appbrewery.com).

`https://cs220.cs.umass.edu/fetch/?url=https://bored-api.appbrewery.com/filter?type=recreational`.
...
`htps://cs220.cs.umass.edu/fetch/noCache/?url=https://bored-api.appbrewery.com/filter?type=recreational`.
